      ...
      m_div(div), m_frame(frame), m_subframe(frame/static_cast<float>(div))
{
    ...
    set_circle(radius, mirrorize(m_boundary, m_circle.getPosition()), color, m_mircle);
...
void Circloid::check_border()
{
    ...
    {
        m_circle.setPosition(mirrorize(m_boundary, m_circle.getPosition()));
    }
}
...
void Circloid::move()
{
    ...
    m_mircle.setPosition(mirrorize(m_boundary, m_circle.getPosition()));
}
